...
if ($role === 'teacher') {
    if ($batchId > 0 && !in_array($batchId, $allowedBatchIds, true)) { $batchId = 0; }
    if ($timingId > 0 && !in_array($timingId, $allowedTimingIds, true)) { $timingId = 0; }
}
...
if ($batchId > 0) { $where[] = 's.batch_id = ?'; $params[] = $batchId; }
if ($timingId > 0) { $where[] = 's.timing_id = ?'; $params[] = $timingId; }
if (in_array($status, ['present','absent','leave'], true)) { $where[] = 'a.status = ?'; $params[] = $status; }
if ($q !== '') { $where[] = 's.fullname LIKE ?'; $params[] = '%' . $q . '%'; }
if ($role === 'teacher') {
                ...
                <form method="get" class="row g-3">
                    ...
                    <div class="col-12 d-flex gap-2">
                        ...
                        <a class="btn btn-outline-success" href="attendance_report.php?<?php 
                            $qp = $_GET; $qp['export'] = 'csv'; echo htmlspecialchars(http_build_query($qp)); 
                        ?>">Export CSV</a>
                    </div>
                </form>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered align-middle">
                        ...
                        <tbody>
                            <?php 
                                $sumPresent = 0; $sumAbsent = 0; $sumLeave = 0; $sumTotal = 0;
                                foreach ($rows as $r):
                                    $total = (int)$r['total_count'];
                                    $present = (int)$r['present_count'];
                                    $absent = (int)$r['absent_count'];
                                    $leave = (int)$r['leave_count'];
                                    $pct = $total > 0 ? round(($present / $total) * 100, 1) : 0.0;
                                    $sumPresent += $present; $sumAbsent += $absent; $sumLeave += $leave; $sumTotal += $total;
                                    $timingLabel = trim(($r['timing_name'] ?? '') . ' ' . ($r['start_time'] ?? '') . ' - ' . ($r['end_time'] ?? ''));
                            ?>
                            <tr>
                                ...
                                <td class="text-end"><?php echo $present; ?></td>
                                <td class="text-end"><?php echo $absent; ?></td>
                                <td class="text-end"><?php echo $leave; ?></td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th class="text-end"><?php echo $sumPresent; ?></th>
                                <th class="text-end"><?php echo $sumAbsent; ?></th>
                                <th class="text-end"><?php echo $sumLeave; ?></th>
                                <th class="text-end"><?php echo $sumTotal; ?></th>
                                <th class="text-end"><?php echo ($sumTotal>0)?round(($sumPresent/$sumTotal)*100,1):0; ?>%</th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
