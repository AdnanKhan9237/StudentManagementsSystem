...
// Fetch academic sessions from the database
$academic_sessions = [];
try {
    $stmt = $db->query("SELECT id, name, start_date, end_date FROM academic_sessions ORDER BY start_date DESC");
    $academic_sessions = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (Throwable $e) {
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($academic_sessions as $session_item): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($session_item['name']); ?></td>
                                    <td><?php echo htmlspecialchars($session_item['start_date']); ?></td>
                                    <td><?php echo htmlspecialchars($session_item['end_date']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
